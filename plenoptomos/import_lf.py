...
def from_lytro(data_path, fname_json, source="warp", mode="grayscale", rgb2gs_mode="luma", binning=1, data_type=np.float32):
    ...
    if mode.lower() == "grayscale":
        ...
        (
            ...
            lf_b.camera.wavelength_range,
        ) = colors.get_rgb_wavelengths()
...
def _flexray_parse_source_det_positions(script_path):
    ...
    for ii_p in range(num_phases):
        ...
        print(
            "binning %d, ROI [%d, %d, %d, %d], [%d, %d = %d x %d] source positions and [%d, %d] detector positions"
            % (
                binnings[ii_p],
                rois[ii_p][0],
                rois[ii_p][1],
                rois[ii_p][2],
                rois[ii_p][3],
                tube_grid_size[ii_p, 0],
                tube_grid_size[ii_p, 1],
                tube_grid_size[ii_p, 1] / tube_grid_size[ii_p, 0],
                tube_grid_size[ii_p, 0],
                det_grid_size[ii_p, 0],
                det_grid_size[ii_p, 1],
            )
        )
    ...
    return (
        ...
        positions_det,
        num_phases,
        np.array(binnings, dtype=np.intp),
        rois,
        out_grid_size[:-1],
        det_is_fixed,
    )
...
def from_flexray(dset_path, crop_fixed_det=True, normalize_flats=False, plot_lf=False, data_type=np.float32):
    ...
    (positions_tube, positions_det, num_phases, binnings, rois, grid_size, det_is_fixed) = _flexray_parse_source_det_positions(
        script_path
    )

    res_uv = np.abs(
        np.array((positions_tube[0, 1, 0] - positions_tube[0, 0, 0], positions_tube[1, 0, 1] - positions_tube[0, 0, 1]))
    )
    ...
    if det_is_fixed and crop_fixed_det:
        ...
        print(
            ...
            % (final_size_ts[0], final_size_ts[1], max_shift_ts[0], max_shift_ts[1]),
            end="",
            flush=True,
        )
        c = tm.time()
        ...
        for ii_v in range(camera.data_size_vu[0]):
            for ii_u in range(camera.data_size_vu[1]):
                ...
                interp_obj = spinterp.RegularGridInterpolator(
                    (base_grid_t, base_grid_s), lf_img[ii_v, ii_u, ...], bounds_error=False, fill_value=0.0
                )
