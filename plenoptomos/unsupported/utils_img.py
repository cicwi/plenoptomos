...
class ShiftsFinderVolume3D(object):
    ...
    def update(self, direction):
        ...
        slices_fix[self.ax_perp] = slice(
                self.ref_pos[self.ax_perp], self.ref_pos[self.ax_perp]+1)
        ...
        self.f.suptitle(self.title % (
                self.planes_vis[self.ax_perp], self.ref_pos[self.ax_perp], 0.0,
                self.shift_vol[2], self.shift_vol[1], self.shift_vol[0]))
        self.f.canvas.draw()
