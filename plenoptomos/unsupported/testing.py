...
class Tester(object):
    ...
    default_rec_params = dict(
        iterations=50, up_sampling_st=1, border=8, psf=None,
        fourier_method='slice', up_sampling_function='flat', data_term='l2')
    ...
    def __init__(
            self, test_name, lfs, test_params={}, rec_params={}, test_conds={},
            is_rgb=False, result_dir=None, save=True):
    ...
    def launch_reconstruction(self, ii_t):
        ...
        if ta.lower() == 'int':
            ...
            func = lambda x, _: refocus.compute_refocus_fourier(
                    x, tz, method=fourier_method, padding_factor=1.125,
                    up_sampling=up_sampling_st, border=border, beam_geometry=tb, domain=td)
        elif ta.lower() == 'bpj':
...
#                    x, tz, algorithm=algo, up_sampling=up_sampling_st, border=border, beam_geometry=tb, domain=td)
            ...
            if ta[:3].lower() == 'itr':
                if ta.lower() == 'itr':
                    algo = solvers.CP_uc(
                        verbose=True, data_term=data_term)
                elif ta.lower() == 'itr_tv':
                    algo = solvers.CP_tv(
                        verbose=True, lambda_tv=lambda_reg, axes=(-2, -1), data_term=data_term)
                    ...
                    algo = solvers.CP_smooth(
                        verbose=True, lambda_d2=lambda_reg, axes=(-2, -1), data_term=data_term)

                func = lambda x, _: tomo.compute_refocus_iterative(
                        x, tz, algorithm=algo, iterations=iterations,
                        up_sampling=up_sampling_st, border=border,
                        up_sampling_function=up_sampling_function,
                        beam_geometry=tb, domain=td)
            else:
                if ta.lower() == 'itp':
                    algo = solvers.CP_uc(
                        verbose=True, data_term=data_term)
                ...
                func = lambda x, p: tomo.compute_refocus_iterative(
                        ...
                        up_sampling=up_sampling_st, border=border,
                        up_sampling_function=up_sampling_function,
                        psf=p, beam_geometry=tb, domain=td)
        ...
        if self.save:
            ...
            imgs_fname = '%s_algo-%s-%s_domain-%s_beam-%s_us-st%d_ds-st%d-uv%d%s.h5' % (
                    self.test_name, ta, data_term, td, tb, up_sampling_st,
...
class Blurring(object):
    ...
    def create_psf_lenses(
        ...
        if coordinates.lower() == 'vu':
            ...
            data_format = None
        self.p[coordinates] = [
            psf.PSFApply2D(psf_d=p, use_otf=use_otf, data_format=data_format, use_fftconv=True)
            for p in self.psfs[coordinates]]
