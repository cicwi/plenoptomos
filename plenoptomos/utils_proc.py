...
def get_smoothing_filter(window_size=(9, 9), window_shape="gauss", plot_filter=False):
...
def remove_background(data, blur_size, blur_func="circ", axes=(0, 1), do_reverse=False, non_negative=False):
...
def get_bandpass_filter(img_shape, cutoff_pix, trans_pix):
    ...
    :rtype: `numpy.array_like`
    """
    return get_lowpass_filter(img_shape, cutoff_pix[0], trans_pix[0]) * get_highpass_filter(
        img_shape, cutoff_pix[0], trans_pix[0]
    )
